...
from dMaSIF.helper import diagonal_ranges
...
def soft_distances(x, y, batch_x, batch_y, smoothness=0.01, atomtype_radii=None):
    ...
    if atomtype_radii is not None:
        ...
        # )  # n_atoms, 1
        smoothness = smoothness*atomtype_radii
...
def atoms_to_points_normals(
    ...
    with torch.enable_grad():
        ...
        dists = soft_distances(
            atoms, z, batch_atoms, batch_z, smoothness=smoothness, atomtype_radii=atomtype_radii
        )
        margin = (dists - T).abs()
        ...
        dists = soft_distances(
            atoms, zz, batch_atoms, batch_z, smoothness=smoothness, atomtype_radii=atomtype_radii
        )
