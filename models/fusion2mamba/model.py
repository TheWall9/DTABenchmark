...
class PoolLayer(nn.Module):
    def __init__(self, dim_in, avg_head=False, max_head=True, attn_head=False):
        super(PoolLayer, self).__init__()
        self.avg_head = avg_head
        self.max_head = max_head
        self.attn_head = attn_head
        if attn_head:
            self.query = nn.Parameter(torch.randn(dim_in, 1))
...
class Fusion2Mamba5(ModelBase):
    ...
    def add_parser_arguments(cls, parser):
        ...
        parser.add_argument("--early_stop_patient", type=int, default=10)
        parser.add_argument("--encoder_n_res_layers", type=int, default=0)
        parser.set_defaults(batch_size=64, lr=1e-4)
...
if __name__ == '__main__':
    ...
    Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name='davis', max_epochs=700, pocket_tops=3,
              ...
              decoder_dropout=0.4,
              protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=True, early_stop_patient=100,
    ...
    exit()
    Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=700, pocket_tops=3,
              # max_head=False, attn_head=False, avg_head=True,
              monitor_metric = 'val/CI', monitor_mode='max',
              use_wandb_logger=True, wandb_project='DTA Benchmark', wandb_online=False,
              encoder_n_res_layers=4,
    ...
    exit()
    dataset_name = "davis"
    # Evaluator(model_name='DeepDTA', deterministic=True, dataset_name=dataset_name, max_epochs=300, batch_size=256, comment='debug', lr=5e-3,
    #           num_workers=16).run(debug=True) # 0.8796136379241943  0.26 270epoch

    # Evaluator(model_name='DeepDTA', deterministic=True, dataset_name=dataset_name, max_epochs=1000, batch_size=256, comment='debug', lr=1e-3, early_stop_patient=200,
    #           num_workers=16).run(debug=True) # 0.8887807130813599  0.23 581epoch

    # Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=300, pocket_tops=2,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           encoder_n_res_layers=4,
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=True, early_stop_patient=100,
    #                  root_data_dir='../../data', decoder_hidden_dims=(2048, 1024, 512), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='debug', lr=5e-3,
    #           num_workers=16).run(debug=True) # 0.8856378197669983  0.227  300epoch

    # Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=300, pocket_tops=2,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           encoder_n_res_layers=4,
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=True, early_stop_patient=100,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 512, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='debug', lr=5e-3,
    #           num_workers=16).run(debug=True) # 0.8791058659553528 0.23  300epoch
    ...
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 512, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='debug', lr=5e-3,
    #           num_workers=16).run(debug=True) # 0.8875152468681335  0.225  300epoch

    # Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=300, pocket_tops=2,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           # encoder_n_res_layers=4,
    #           protein_encoder_n_layers=2, ligand_encoder_n_layers=2, bidirectional=False, merge_train_val=True, early_stop_patient=100,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 512, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='debug', lr=5e-3,
    #           num_workers=16).run(debug=True) # 0.8844261765480042  0.242 300epoch
    ...
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=True, early_stop_patient=100,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 512, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='debug', lr=1e-3,
    #           num_workers=16).run(debug=True) # 0.8900253176689148 0.22 300epoch

    # Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=1000, pocket_tops=3,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           encoder_n_res_layers=4,
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=True, early_stop_patient=100,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 512, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='debug', lr=1e-3,
    #           num_workers=16).run(debug=True) # 0.8876829147338867  0.22  641epoch

    # Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=1000, pocket_tops=3,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           encoder_n_res_layers=4,
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=True, early_stop_patient=200,
    #           root_data_dir='../../data', decoder_hidden_dims=(2048, 1024, 512), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='debug', lr=1e-3,
    #           num_workers=16).run(debug=True) # 0.8869603276252747  0.21  805epoch
    ...
    #           root_data_dir='../../data', decoder_hidden_dims=(2048, 1024, 512), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='debug', lr=1e-3,
    #           num_workers=16).run(debug=True) #  0.8889332413673401  0.225178986787796 839epoch

    # Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=1000, pocket_tops=2,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           # encoder_n_res_layers=4,
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=True, early_stop_patient=200,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 512, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=256, batch_size=256, comment='debug', lr=1e-3,
    #           num_workers=16).run(debug=True) #
    ...
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=True, early_stop_patient=200,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 512, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='exp1', lr=1e-3,
    #           num_workers=16).run() # 0.885871 0.22

    dataset_name = 'davis_pocketdta'
    # dataset_name = "davis2"
    # Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=1000, pocket_tops=2,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           # encoder_n_res_layers=4,
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=False, early_stop_patient=200,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 1024, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='exp2', lr=1e-3,
    #           num_workers=16).run() # 0.907 0.179

    dataset_name = 'davis_pocketdta'
    # Evaluator(model_name='Fusion2Mamba5Scale', deterministic=True, dataset_name=dataset_name, max_epochs=200, pocket_tops=2,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           # encoder_n_res_layers=4,
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=False, early_stop_patient=200,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 1024, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='exp2', lr=1e-3,
    #           num_workers=16).run(debug=True) # 0.8918 0.190
    ...
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=False, early_stop_patient=200,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 1024, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=32, comment='exp2', lr=1e-3,  accumulate_grad_batches=8,
    #           num_workers=16).run(debug=True) #0.890 0.194
    ...
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 1024, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=32, comment='exp2', lr=1e-3,  accumulate_grad_batches=8,
    #           num_workers=16).run(debug=True) #step lr 0.899 0.187

    dataset_name = 'davis_refine'
    # Evaluator(model_name='Fusion2Mamba5', deterministic=True, dataset_name=dataset_name, max_epochs=1000, pocket_tops=2,
    #           # max_head=False, attn_head=False, avg_head=True,
    #           # encoder_n_res_layers=4,
    #           protein_encoder_n_layers=1, ligand_encoder_n_layers=1, bidirectional=False, merge_train_val=False, early_stop_patient=200,
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 512, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=256, comment='exp1', lr=1e-3,
    #           num_workers=16).run() #  0.883648  0.204259
    ...
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 1024, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=32, comment='exp2', lr=1e-3,  accumulate_grad_batches=8,
    #           num_workers=12).run(debug=True) #0.878  0.23
    ...
    #           root_data_dir='../../data', decoder_hidden_dims=(1024, 1024, 256), pretrained_model_name_or_path='facebook/esm2_t6_8M_UR50D',
    #           encoder_embedding_dim=128, batch_size=32, comment='exp2', lr=1e-3,  accumulate_grad_batches=8,
    #           num_workers=12).run(debug=True) #step lr 0.880 0.238
